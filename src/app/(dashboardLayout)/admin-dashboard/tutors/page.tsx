...
export default function TutorPage() {
  ...
  const [fetchTutor, setFetchTutor] = useState(true);

  useEffect(() => {
    async function fetchAllStudent() {
      const result = await getUsersAction();
      const allTutors = result.data.data.filter(
        (tutor: any) => tutor.role === Roles.tutor,
      );
      setTutors(allTutors);
    }
    fetchAllStudent();
  ...
  const handleActive = async (id: string) => {
    if (confirm("Are you sure you want to active the Tutor?")) {
      const data = {
          ...
          <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
            {tutors?.map((tutor: any) => (
              ...
              >
                ...
                <td className="px-6 py-4">{tutor.phone ?? "N/A"}</td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    {tutor.status === Status.banned ? (
                      <button
                        onClick={() => handleActive(tutor?.id)}
